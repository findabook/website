...
router.post('/add', (req, res) => {
  ...
  categories = categories.filter((categorie, i) => categories.indexOf(categorie) === i)
  books.findOne({ name: req.body.name })
    .then(book => {
      if (book == null) {
        books.create({
          name: req.body.name,
      ...
      books.findOneAndUpdate({ name: req.body.name }, { $inc: {quantity: 1}, $push: {actualOwners: req.user._id} },{new: true})
